...
class GameBoard:
    ...
    def minmax(self, bd=None, depth=0):
        ...
        moves = [(r, c) for r in range(3) for c in range(3) if
                 bd[r][c] == 0]  # all possible position where the board is empty
...
class TicTacToeGame:
    ...
    def playturn(self):
        ...
        # I removed alphabeta because this is what I got to work :)
        nicetry = 0  # This is not necessary for the code just for my own fun.
        if self.turn == 1:
            print("Human, please choose a space!")
            while True:
                user_input = input("Enter two numbers separated by a comma: ")
                try:
                    spot = user_input.split(',')
                    if len(spot) != 2:
                        raise ValueError("Sorry! Please try again! We need exactly two values: row,col")
                    ...
                    if nicetry < 3:
                        ...
                        print("Quit trying to break my code and just play the game!!!")
        else:
            print("AI is thinking...")
            move, score = self.gameboard.alphabeta(self.gameboard.entries, 0, -math.inf, math.inf)
